...
This document provides guidance for AI tools and developers using AI assistance when contributing to Euro-Office. It applies to all repositories under the [Euro-Office GitHub organization](https://github.com/Euro-Office/), including DocumentServer and all component repositories (server, web editors, SDK, and native core).

This policy complements the existing [Contribution Guidelines](CONTRIBUTING.md). The requirements around testing, sign-off, license headers, and security reporting described there continue to apply in full - this document addresses how they extend to AI-assisted contributions.
...
For general security requirements applicable to all contributions, see the [Contribution Guidelines](CONTRIBUTING.md). Security vulnerabilities must be reported privately by email to the maintainers (`euro-office-team` on the `proton.me` mail server), not via public issues. AI-generated security reports must be independently verified before submission; unverified reports might be closed without response.
...
AI agents must not add `Signed-off-by` tags: only humans can legally certify the [Developer Certificate of Origin](https://developercertificate.org/). Euro-Office requires every commit to be signed off; this certification is the contributor's alone.

### Licensing and Copyright Compliance

Euro-Office is licensed under the [AGPLv3](https://www.gnu.org/licenses/agpl-3.0.en.html). Contributors must ensure AI-generated code contains no material from sources incompatible with that license, or with the license of any third-party component they are contributing to.
...
Signs of a disallowed submission include: large unreviewed AI blobs; obvious mechanical mistakes a human would fix in minutes; code that has clearly never been executed; and pull requests that shift debugging and cleanup work onto maintainers rather than the contributor. As required by the [Contribution Guidelines](CONTRIBUTING.md), all changed and added code must be tested - AI-generated code is not exempt from this requirement.

New features must be tested on a live Euro-Office instance by the contributor before submission. Providing test instructions for an AI agent to execute is not a substitute for human testing.
...
AI tools must not be used to fully automate the resolution of issues labeled [`good first issue`](https://github.com/search?q=org%3AEuro-Office+label%3A%22good+first+issue%22&type=issues) or similar beginner-friendly labels. These issues exist to create a learning path for new contributors who will become future maintainers. Take a beginner issue only if you genuinely intend to learn and continue contributing to Euro-Office.

---

## Scope and Updates

This policy applies to all contributions to repositories under the Euro-Office GitHub organization, by all contributors. It will be reviewed and updated as AI tooling, open-source best practices, and applicable law evolve. Suggested changes are welcome via pull requests.
